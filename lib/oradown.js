...
var oradown = function(accept_license, file, outDir, outFile, oralogin, orapwd, url, showbrowser) {
    ...
    nightmare.on('download', function(state, downloadItem) {
        if (state == 'started') {
            ...
            var toPath = path.resolve(toFile);
            console.log('Downloading: ' + downloadItem.url + ' to: ' + toPath);

            var mkdirp = require('mkdirp');
            mkdirp( path.dirname(toPath), function(err) {
                if (err) {
                    console.error(err)
                } else {
                    console.log('mkdirp succeeded for: ' + path.dirname(toPath))
                }
            });

            nightmare.emit('download', toPath, downloadItem);
        }
    });
